...
async function seedContent() {
    try {
        const adminRes = await pool.query("SELECT id FROM admin_users WHERE email = 'admin@tcil.net.in'");
        const adminId = adminRes.rows.length > 0 ? adminRes.rows[0].id : null;

        await pool.query(`
            INSERT INTO news_articles (title_en, title_hi, body_en, body_hi, status, author_id)
            VALUES (
        ...
        console.log('✅ Dummy content added successfully.');
        process.exit(0);
    } catch (error) {
